...
Tim::BaseImagesController.class_eval do
  ...
  def set_targets
    ...
    @targets = provider_types.map do |provider_type|
      # use last - there can be multiple target images with failed status
      target_image = target_images.select{|ti| ti.provider_type_id == provider_type.id}.last
      get_target_info(target_image, provider_type, all_prov_accts)
    end
  end
  ...
  def get_provider_image_data(provider_image, provider_account, target_image)
    ...
    if (provider_image.nil? || provider_image.status == Tim::ProviderImage::STATUS_FAILED) &&
      check_privilege(Privilege::MODIFY, @base_image) && target_image
        ...
        provider_image_data[:pimg_push_url] = tim.provider_images_path(:provider_image => {
      ...
      check_privilege(Privilege::MODIFY, @base_image)
        ...
        provider_image_data[:pimg_delete_url] = tim.target_image_path(target_image.id)
    else
  ...
  def get_target_info(target_image, provider_type, all_prov_accts)
    info = {
    ...
    provider_accounts.each do |provider_account|
      provider_image = target_image.provider_images.where(
        :provider_account_id => provider_account.id).last if target_image.present?
      info[:provider_images] << get_provider_image_data(provider_image, provider_account, target_image)
    end
